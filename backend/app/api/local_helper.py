...
router = APIRouter(prefix="/local-helper", tags=["local-helper"])
...
def _root_dir() -> Path:
    # Adjusted to point to the project root from backend/app/api/local_helper.py
...
def health() -> dict[str, str]:
    return {"status": "ok", "service": "xhs-local-helper-integrated", "bind": "integrated"}
...
def workbench() -> Response:
    ...
    # Update HTML to use the integrated paths
    html = html.replace("const helperBase = window.location.origin;", "const helperBase = window.location.origin + '/api/local-helper';")
...
@router.post("/xhs/cookies/push-to-server")
