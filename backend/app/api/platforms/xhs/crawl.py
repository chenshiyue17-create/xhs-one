...
def crawl_note_urls(
    ...
    from backend.app.adapters.xhs.scrapling_adapter import XhsScraplingAdapter
    scrapling_adapter = XhsScraplingAdapter(cookies)
    ...
    for url in payload.urls:
        ...
        # Try original adapter first
        success, message, raw_payload = adapter.get_note_info(url)
        
        # Fallback to Scrapling if original fails
        if not success:
            logger.warning(f"Original adapter failed for {url}, trying Scrapling fallback: {message}")
            success, message, raw_payload = scrapling_adapter.get_note_info(url)
                ...
                for page in range(1, payload.pages + 1):
                    ...
                    for raw_item in _data_items(raw_payload):
                        ...
                        if note_url:
                            ...
                            if not ds:
                                from loguru import logger
                                logger.warning(f"Original adapter failed for {note_url}, trying Scrapling fallback: {dm}")
                                ds, dm, dp = scrapling_adapter.get_note_info(note_url)
